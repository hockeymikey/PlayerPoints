...
public class PlayerPointsAPI {
     ...
     */
    public PlayerPointsAPI(final PlayerPoints p) {
        this.plugin = p;
    }
     ...
     */
    public boolean give(final UUID playerId, final int amount) {
        if (playerId == null) {
            return false;
        }
        final PlayerPointsChangeEvent event = new PlayerPointsChangeEvent(playerId,
                                                                          amount);
    ...
    @Deprecated
    public boolean give(final String playerName, final int amount) {
     ...
     * @return True if we successfully adjusted points, else false
     */
    public boolean take(final UUID playerId, final int amount) {
    ...
    @Deprecated
    public boolean take(final String playerName, final int amount) {
        boolean success = false;
        if (playerName != null) {
     ...
     */
    public int look(final UUID playerId) {
        int amount = 0;
        if (playerId != null) {
    ...
    @Deprecated
    public int look(final String playerName) {
     ...
     * @return True if we successfully adjusted points, else false
     */
    public boolean pay(final UUID source, final UUID target, final int amount) {
    ...
    @Deprecated
    public boolean pay(final String sourceName, final String targetName, final int amount) {
        boolean success = false;
     ...
     */
    public boolean set(final UUID playerId, final int amount) {
        if (playerId == null) {
            return false;
        }
        final PlayerPointsChangeEvent event = new PlayerPointsChangeEvent(playerId,
    ...
    @Deprecated
    public boolean set(final String playerName, final int amount) {
        boolean success = false;
        if (playerName != null) {
    ...
    public boolean reset(final UUID playerId) {
        ...
        final PlayerPointsResetEvent event = new PlayerPointsResetEvent(playerId);
        plugin.getServer().getPluginManager().callEvent(event);
        if (!event.isCancelled()) {
            return plugin.getModuleForClass(StorageHandler.class).setPoints(
    ...
    @Deprecated
    public boolean reset(final String playerName, final int amount) {
        boolean success = false;
        if (playerName != null) {
