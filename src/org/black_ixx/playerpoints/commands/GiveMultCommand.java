 ...
 * @author Mitsugaru
 */
public class GiveMultCommand implements PointsCommand {
    ...
    public boolean execute(PlayerPoints plugin, CommandSender sender,
        ...
        if(!PermissionHandler.has(sender, PermissionNode.GIVEMULT)) {
            info.put(Flag.EXTRA, PermissionNode.GIVEMULT.getNode());
        ...
        try {
            ...
            final int gsfdgg = Integer.parseInt(args[2]);
            final int result = anzahl * gsfdgg;
            ...
            if(plugin.getAPI().give(id, result)) {
