...
public abstract class CommandHandler implements CommandExecutor {
     ...
     */
    public CommandHandler(final PlayerPoints plugin, final String cmd) {
        this.plugin = plugin;
        this.cmd    = cmd;
    }
     ...
     *                command.
     */
    public void registerCommand(final String label, final PointsCommand command) {
    ...
    public void registerHandler(final CommandHandler handler) {
        if (registeredHandlers.containsKey(handler.cmd)) {
            plugin.getLogger().warning(
                    "Replacing existing handler for: " + handler.cmd);
        }
        registeredHandlers.put(handler.cmd, handler);
    }
     ...
     */
    public void unregisterHandler(final String label) {
    ...
    public boolean onCommand(final CommandSender sender, final Command command,
        ...
        final EnumMap<Flag, String> info = new EnumMap<>(Flag.class);
     ...
     */
    protected String[] shortenArgs(final String[] args) {
        if (args.length == 0) {
            return args;
        }
